...
static void solve() throws Exception {
     ...
     *
     * 1 ... n       -> original nodes
     * n+1 ... n+d   -> virtual nodes
     */
    int totalNodes = n + d;

    graph = new ArrayList<>(totalNodes + 1);

    for (int i = 0; i <= totalNodes; i++) {
        graph.add(new ArrayList<>());
    }

    // Adjacent nodes: cost b
    for (int i = 1; i < n; i++) {
        graph.get(i).add(new Edge(i + 1, b));
        graph.get(i + 1).add(new Edge(i, b));
    }
     ...
     *     v1    v2    v3
     *
     * Going from an original node -> virtual node costs 0
     * Going from virtual node -> original node costs a
     */
    int currD = n + 1;

    for (ArrayList<Integer> vertices : map.values()) {

        for (int v : vertices) {
            graph.get(v).add(new Edge(currD, 0));
            graph.get(currD).add(new Edge(v, a));
        }

        currD++;
    }

    // Number of nodes is n + d
    dist = new long[totalNodes + 1];
    ...
    // Only print distances to original nodes
    for (int i = 1; i <= n; i++) {
